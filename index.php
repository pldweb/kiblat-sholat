...
<script>
    $(document).ready(function() {
        if (window.DeviceOrientationEvent) {
            $('#notice').html('Working API detected');
            
            $(window).on('deviceorientation', function(eventData) {
                const tiltLR = eventData.originalEvent.gamma;
                const tiltFB = eventData.originalEvent.beta;
                const dir = eventData.originalEvent.alpha;
                deviceOrientationHandler(tiltLR, tiltFB, dir);
            });
        } else {
            $('#notice').html('No API detected');
        }

        function deviceOrientationHandler(tiltLR, tiltFB, dir) {
            $('#tiltLR').html(Math.ceil(tiltLR));
            $('#tiltFB').html(Math.ceil(tiltFB));
            $('#direction').html(Math.ceil(dir));

            const compassDisc = $('#compassDiscImg');
            compassDisc.css({
